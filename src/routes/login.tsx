...
export const Route = createFileRoute("/login")({
  head: () => ({ meta: [{ title: "Log in — getWorkers" }] }),
  component: Login,
});
...
function Login() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    setLoading(false);
    nav({ to: profile?.role === "company" ? "/company" : "/worker" });
  }
  ...
  return (
    <AppShell role="public">
      <div className="mx-auto max-w-md px-4 py-12 md:px-6">
        ...
        <p className="mt-1 text-muted-foreground">Welcome back to getWorkers.</p>
        ...
        <form onSubmit={onSubmit} className="mt-8 space-y-4 rounded-xl border border-border bg-card p-6">
          ...
          <button disabled={loading} className="w-full rounded-md bg-primary py-2.5 font-semibold text-primary-foreground hover:opacity-90 disabled:opacity-60">
          ...
          <div className="text-center text-sm text-muted-foreground">
            ...
            {" · "}
            <Link to="/signup/company" className="font-medium text-primary hover:underline">Company</Link>
          </div>
        </form>
      </div>
    </AppShell>
